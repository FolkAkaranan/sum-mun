...
import type { CustomState, PlayerState } from "@/lib/types";
    ...
    <div className="flex flex-1 flex-col items-center justify-between gap-6 py-4">
      <div className="flex flex-1 flex-col items-center justify-center gap-4">
        <button
          ...
          aria-label="สุ่มจากรายการที่กำหนดเอง"
        >
          📝
        </button>
        <p className="text-sm text-neutral-400">
          {state.items.length === 0
            ? "ยังไม่มีรายการ กดตั้งค่าเพื่อเพิ่มเนื้อหาของคุณเอง"
            : "แตะเพื่อสุ่มจากรายการที่คุณเพิ่มเอง"}
        </p>
        {turn.current && (
      ...
      {state.lastDrawn && (
        <div className="fixed inset-0 z-40 flex items-center justify-center overflow-y-auto bg-black/50 px-4 py-6">
          <div className="flex w-full max-w-xs flex-col items-center gap-4 rounded-2xl bg-white p-6 text-center dark:bg-neutral-900">
            <p className="text-xs uppercase tracking-wide text-indigo-500">กำหนดเอง</p>
            <p className="text-xl font-bold">{state.lastDrawn.text}</p>
            {state.lastPlayer && (
              <p className="text-sm text-neutral-500">
                👤 คิวนี้: <span className="font-semibold">{state.lastPlayer}</span>
      ...
      {settingsOpen && (
        <Modal title="จัดการรายการที่กำหนดเอง" onClose={() => setSettingsOpen(false)}>
          <form onSubmit={addItem} className="mb-2 flex gap-2">
            <input
              value={text}
              onChange={(e) => setText(e.target.value)}
              placeholder="เพิ่มรายการ... (คั่นด้วย , เพื่อเพิ่มหลายอันพร้อมกัน)"
          ...
          {state.items.length > 0 && (
            <div className="mb-4 flex justify-end">
              <button
                onClick={() => {
                  if (confirm("ลบรายการทั้งหมด?")) onClearAll();
                }}
