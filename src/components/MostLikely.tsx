...
import type { MostLikelyState, PlayerState } from "@/lib/types";
...
export default function MostLikely({
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-between gap-6 py-4">
      <div className="flex flex-1 flex-col items-center justify-center gap-4">
        ...
        <p className="text-sm text-neutral-400">
          {state.items.length === 0
            ...
            : "แตะเพื่อสุ่มคำถาม \"ใครมีแนวโน้มจะ...\""}
        </p>
        {turn.current && (
      ...
      {state.lastDrawn && (
        <div className="absolute inset-0 z-40 flex items-center justify-center bg-black/50 px-4">
          <div className="flex w-full max-w-xs flex-col items-center gap-4 rounded-2xl bg-white p-6 text-center dark:bg-neutral-900">
            <p className="text-xs uppercase tracking-wide text-amber-500">ใครมีแนวโน้มจะ...</p>
            ...
            <div className="flex w-full gap-3">
              ...
              <button
                onClick={onDraw}
                className="flex-1 rounded-xl bg-amber-600 py-2.5 font-medium text-white hover:bg-amber-700"
              >
                สุ่มใหม่
              </button>
            </div>
      ...
      {settingsOpen && (
        <Modal title="จัดการคำถาม ใครมีแนวโน้มจะ" onClose={() => setSettingsOpen(false)}>
